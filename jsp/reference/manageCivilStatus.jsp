...
<body>
	...
	<div id="page_container">
		<div id="breadcrumb_area">
			<div id="breadcrumb">
				<ul>
					...
					<li><spring:message code="REF.UI.MANAGE.CIVILSTATUS.TITLE" />
					</li>
				</ul>
			</div>
			<div class="help_link">
				<a href="#"
					onClick="PopupCenterScroll('<c:url value="resources/html/akura-help/topics/admin/manageCivilStatusHelp.html"/>','helpWindow',780,550)"><img
		...
		<h1>
			<spring:message code="REF.UI.MANAGE.CIVILSTATUS.TITLE" />
		</h1>
		<div id="content_main">
			<form:form action="manageSaveOrUpdateCivilStatus.htm" method="post"
				commandName="civilStatus" name="form">

				<div class="section_box">
					<div id="search_results">
						<div class="section_box_header">
							<h2>
								<spring:message code="REF.UI.MANAGE.CIVILSTATUS.MAIN.TITLE" />
							</h2>
						</div>
						<div id="area">
							...
							<table class="basic_grid" border="0" cellspacing="0"
								...
								<tr>
									<th width="830"><spring:message
											code="REF.UI.MANAGE.CIVILSTATUS.TYPE" />
									</th>
									<th width="78" class="right"><sec:authorize
											access="hasRole('Save/Update Civil Status')">
              ...
              "
												width="18" height="20"
												title="<spring:message code="REF.UI.MANAGE.CIVILSTATUS.ADD"/>">
										</sec:authorize>
									</th>
								</tr>
								<c:choose>
									<c:when test="${not empty civilStatusList}">
										<c:forEach var="civilStatus" items="${civilStatusList}"
											varStatus="status">
											<tr
												<c:if test="${selectedObjId != null && (selectedObjId == civilStatus.civilStatusId)}">class="highlight"</c:if>
												<c:choose>
            	...
            	<c:otherwise>
            		class="even"
												...
												<td><c:out value="${civilStatus.description}"></c:out>
												</td>
												<td nowrap class="right"><sec:authorize
														access="hasRole('Save/Update Civil Status')">
														<img src="resources/images/ico_edit.gif"
															title="<spring:message code="REF.UI.MANAGE.CIVILSTATUS.EDIT"/>"
															onClick="updateCivilStatus(this, '<c:out value="${civilStatus.civilStatusId}" />', '<c:out value="${strEscapeUtil:escapeJS(civilStatus.description)}" />' ,showArea());"
															width="18" height="20" class="icon">
													</sec:authorize> <sec:authorize access="hasRole('Delete Civil Status')">
														<img src="resources/images/ico_delete.gif"
															onClick="showArea(),document.form.civilStatusId.value='${civilStatus.civilStatusId}';
                      deleteCivilStatus(this)"
									...
									<c:otherwise>
										<tr>
											<td width="830">
												<h5>
													<spring:message
														code="REF.UI.MANAGE.CIVILSTATUS.NO.CIVILSTATUS.TYPE.FOUND" />
												</h5></td>
					...
					<div class="section_full_inside" style='display:${editpane!=null?'block':'none' }'>
						<c:choose>
							<c:when test="${selectedObjId > 0 }">
								<h3>
									<spring:message code="REF.UI.MANAGE.CIVILSTATUS.EDIT" />
								</h3>
							</c:when>
							<c:otherwise>
								<h3>
									<spring:message code="REF.UI.MANAGE.CIVILSTATUS.ADD" />
								</h3>
							</c:otherwise>
						</c:choose>
								...
								<div class="float_left">
									<div class="lbl_lock">
										<label><span class="required_sign">*</span> <spring:message
												code="REF.UI.MANAGE.CIVILSTATUS.TYPE" />:</label>
									</div>
