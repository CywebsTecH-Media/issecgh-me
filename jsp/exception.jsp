				...
				<div class="row">
					...
					<div class="errorMsg">					
						<c:if test="${pageNotFound != null}">
				     		<spring:message code="ERROR.DESCRIPTION.TITLE"/>: ${pageNotFound}.
				     	</c:if>
				     	<c:if test="${accessDenied != null}">
				     		<spring:message code="ERROR.DESCRIPTION.TITLE"/>: ${accessDenied}.
				     	</c:if>
				     	<c:if test="${methodNotAllowed != null}">
				     		<spring:message code="ERROR.DESCRIPTION.TITLE"/>: ${methodNotAllowed}.
				     	</c:if>
				     	<c:if test="${smsAppException != null}">
				     		<spring:message code="ERROR.DESCRIPTION.TITLE"/>: ${smsAppException}.
				     	</c:if>
				     	<c:if test="${dbConnectionError != null}">
				     		<spring:message code="ERROR.DESCRIPTION.TITLE"/>: ${dbConnectionError}.
				     	</c:if>	
				     	<c:if test="${akuraAppException != null}">
				     		<spring:message code="ERROR.DESCRIPTION.TITLE"/>: ${akuraAppException}.
				     	</c:if>	
					</div>
				...
				${exception.getMessage()}				
				 -->
					<div style="line-height:17px;">
						...
						<a href= "mailto: <spring:message code="ERROR.SUPPORT.EMAIL"/>?subject=<spring:message code="ERROR.EMAIL.SUBJECT"/>&body=<spring:message code="ERROR.EMAIL.BODY"/> <spring:message code="APPLICATION.URL"/>." ><i><b><spring:message code="ERROR.SUPPORT.EMAIL"/></b></i> </a>


						  <spring:message code="ERROR.SUPPORT.DESCRIPTION.PART2"/>
					</div>
