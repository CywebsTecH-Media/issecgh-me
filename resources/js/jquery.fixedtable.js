...
(function($) {
    ...
    $.fn.fixedTable = function(opts) {
        ...
        options.fixedColumnWidth = $('.'+options.classColumn,layout).width();
        
        // keep align header and table body cells 

        var tbodycount =0;
        var stdNameCellWidth =0;
        var addmissionNoCellWidth =0;
        var stdCountCellWidth =0;
       
        $("." + options.classColumn + "> .fixedTable" + "> table > tbody >  tr:first > td", layout).each(function() {
        	
        	if(tbodycount==0){
        		stdCountCellWidth = $(this).width();
        		$(this, layout).css({
		            "padding": "0 0 1px 4px"
        	    });
        	}
        	if(tbodycount==1){
        		addmissionNoCellWidth = $(this).width();
        		$(this, layout).css({
		            "padding": "0 0px 1px 4px"
        	    });
        	}
        	if(tbodycount==2){
        		stdNameCellWidth = $(this).width();
        		$(this, layout).css({
		            "padding": "0 0 1px 4px"
        	    });
        	}
        ...
        $("." + options.classColumn + ">  ." + options.classHeader + " >table > tbody > tr > td", layout).each(function() {
        	if(Hcount==0){
        		$(this).width(stdCountCellWidth);
        		$(this, layout).css({
		            "padding": "0 0 1px 4px"
        	    });
        	}
        	if(Hcount==1){
        		$(this).width(addmissionNoCellWidth);
        		$(this, layout).css({
		            "padding": "0 0 1px 4px"
        	    });
        	}
        	if(Hcount==2){
        		$(this).width(stdNameCellWidth);
        		$(this, layout).css({
		            "padding": "0 0 1px 4px"
        	    });
        	}
       ...
       // $("." + options.classColumn, layout).width(options.fixedColumnWidth);
        ...
        //equle fixedcontainer th height and fixedcolume th height
        var tdH = $(".fixedContainer .fixedHead  > table > tbody > tr > td").height();        
        $(".fixedColumn .fixedHead > table > tbody > tr > td").height(tdH);
    ...
    function buildFixedColumns(src, section, cols, target) {
        ...
        if ($(section, src).length) {
            ...
            $(section + " tr", src).each(function() {
				...
				if(countS == 2){
				$("<td>" + $(this).html() + "</td>").width('auto').addClass(this.className).attr("id", this.id).appendTo(tr);
				}else{
                    $("<td>" + $(this).html() + "</td>").width(widthheader[countS]).addClass(this.className).attr("id", this.id).appendTo(tr);
					}
